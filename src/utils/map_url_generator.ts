...
import { MAP_URL_ZOOM, SURVER_YEAR } from './const';

type LayerCondition = (p: any) => boolean;
type LayerParams = (p: any) => string[];
...
function getArray(p: any, key: string): string[] {
    const val = p[key];
    if (!val) return [];
...
export const API_LAYERS: Record<number, LayerEntry[]> = {
    3: [
        {
            layer: "publicNotices",
            condition: p => {
            ...
            params: p => {
                const codes = getArray(p, "use_category_code");
                if (codes.length > 0) return codes.map(c => `publicNotices=${c}`);
    ...
    12: [
        ...
        ...[
            ...
            condition: (p: any) => {
                ...
                if (codes.length === 0) return true; // defaults to true if no codes specified
...
export function buildMapUrl(lat: number, lon: number, targetApis: number[], params: any = {}): string {
    ...
    if (targetApis.includes(3)) {
        ...
        if (!isNaN(year) && Number.isInteger(year)) {
            ...
            urlParams.push(`surveyYear=${SURVER_YEAR}`);
        }
    }

    for (const api of targetApis) {
        const entries = API_LAYERS[api] || [];
    ...
    urlParams.push(`zoom=${MAP_URL_ZOOM}`);
