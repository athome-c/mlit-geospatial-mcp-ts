...
const toolsJson = [
      ...
      "inputSchema": {
        ...
        "properties": {
          ...
          "target_apis": {
            ...
            "description": "呼び出すAPI番号。(例：[3,4,5])未指定または空配列の場合は、全APIを取得します。(例：[])"
          },
          "distance": {
            "type": "number",
            "description": "中心地点からの検索距離（メートル）。（任意）この値は 0〜425[m] の範囲で指定できます。425m を超える場合はエラーになります。target_apiが1,2,3,9,10,11,12,15,17,18のとき有効。",
            "default": 425.0,
            "maximum": 425.0
          },
          "price_classification": {
            "type": "string",
            "description": "target_apiが1のとき有効。価格情報区分コード（任意）。\n  \"01\": 不動産取引価格情報のみ, \"02\": 成約価格情報のみ, 未指定: 不動産取引価格情報と成約価格情報の両方"
          },
          "year": {
            "type": "number",
            "description": "対象年（4桁の西暦）。1995〜最新年の範囲で指定可能です。未指定の場合は 2024 年として処理します。target_apiが1,2,3のとき有効。"
          },
          "quarter": {
            "type": "number",
            "description": "target_apiが1のとき有効。取引時期（四半期）。（任意）\n  \"1\":1月～3月,\"2\":4月～6月,\"3\":7月～9月,\"4\":10月～12月"
          },
          "land_price_classification": {
            "type": "string",
            "description": "target_apiが3のとき有効。地価情報区分コード。（任意）\n  \"0\"  : 国土交通省地価公示のみ\n  \"1\"  : 都道府県地価調査のみ\n  (未指定の場合は、地価公示＋地価調査の両方）",
            "enum": [
              "0",
              "1"
            ]
          ...
          "language": {
            "type": "string",
            "description": "target_apiが1のとき有効。出力結果の言語。（任意）\n  \"ja\"  : 日本語\n  \"en\"  : 英語\n  (未指定の場合は、日本語）"
          },
          "division": {
            "type": "string",
            "description": "target_apiが2のとき有効。用途区分（任意）。カンマ区切り指定可。\n  \"00\": 住宅地, \"03\": 宅地見込地, 05: 商業地, 07: 準工業地,\n  09: 工業地, 10: 市街地調整区域内現況宅地,\n  13: 現況林地, 20: 林地（都道府県地価調査）,未指定：すべて"
          },
          "use_category_code": {
            "type": "array",
            "items": {
            ...
            "description": "target_apiが3のとき有効。用途区分コード（任意）。カンマ区切り指定可。\n  \"00\": 住宅地, \"03\": 宅地見込地, 05: 商業地, 07: 準工業地,\n  09: 工業地, 10: 市街地調整区域内現況宅地,\n  13: 現況林地（公示のみ）, 20: 林地（調査のみ）,未指定：すべて"
          },
          "administrative_area_code": {
            ...
            "description": "target_apiが7,8,12,16,17,21,22のとき有効。行政区域コード（任意）。カンマ区切り指定可。 形式は数字5桁（文字列）"
          },
          "welfare_facility_class_code": {
            ...
            "description": "target_apiが12のとき有効。福祉施設大分類コード（任意）。カンマ区切り指定可。\n  01: 保護施設, 02: 老人福祉施設, 03: 障害者支援施設等, 04: 身体障害者社会参加支援施設,\n  05: 児童福祉施設等, 06: 母子・父子福祉施設,99: その他の社会福祉施設等 形式は数字2桁（文字列）"
          },
          "welfare_facility_middle_class_code": {
            ...
            "description": "target_apiが12のとき有効。福祉施設中分類コード（任意）。カンマ区切り指定可。 形式は数字4桁（文字列）"
          },
          "welfare_facility_minor_class_code": {
            ...
            "description": "target_apiが12のとき有効。福祉施設小分類コード（任意）。カンマ区切り指定可。 形式は数字6桁（文字列）"
          },
          "prefecture_code": {
            "type": "array",
            "items": {
            ...
            "description": "target_apiが19, 21, 22のとき有効。都道府県コード（任意）。カンマ区切り指定可。 形式は数字2桁（文字列）"
          },
          "district_code": {
            "type": "array",
            "items": {
            ...
            "description": "target_apiが19のとき有効。地区コード（任意）。カンマ区切り指定可。 形式は数字1桁または数字2桁（文字列）。1桁目が0のものについては、0を取り除いた値で指定。"
          },
          "save_file": {
            "type": [
              "boolean",
            ...
            "description": "取得結果のgeojsonファイルを任意のフォルダに保存するかどうかのフラグ。指定がない場合はNoneとする。true：ファイル保存する、false:保存しない、None：ユーザ確認。save_fileがNoneの場合は、必ず「取得結果のファイルは保存しますか？」とユーザーに確認してください。保存する場合（save_file=true）は保存先フォルダをユーザに返却する。"
          },
          "output_dir": {
            "type": "string",
  ...
  },
  {
    "tool": {
      ...
      "description": "入力された住所や地名（例: 東京都新宿区西新宿2-8-1, 東京タワーなど）から、緯度(lat)と経度(lon)を取得します。get_multi_api ツールや plateau_space_id ツールを使用する際に、ユーザーから緯度経度ではなく住所のみを与えられた場合は、まずこのツールで緯度経度を取得してください。（内部的に国土地理院のジオコーディングAPIを使用しています）",
      "inputSchema": {
        "type": "object",
        "properties": {
          "address": {
            "type": "string",
            "description": "緯度経度に変換したい住所やランドマーク名。例: 愛知県名古屋市中区上前津１丁目"
          }
        },
        "required": [
          "address"
...
export const TOOLS: any[] = toolsJson.map((p: any) => p.tool);
export const API_SPECS: { [key: string]: ApiSpec } = {};

for (const p of toolsJson) {
  API_SPECS[p.spec.tool_name] = {
    ...p.spec,
